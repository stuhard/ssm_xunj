<template>
  <view class="page_patrol_personnel diy_table diy_table_tml" id="patrol_personnel_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/patrol_personnel/view?" v-show="user_group == '管理员' || $check_action('/patrol_personnel/table','add') || $check_action('/patrol_personnel/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="0 && $check_field('get','personnel_name')" class="col">
						<view class="view">
                            ...
                            <span>人员姓名</span>
								...
								<text>
									{{ o["personnel_name"] }}
													...
													<view v-if="0 && $check_field('get','gender_of_personnel')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>人员性别</span>
                          </view>
											<view class="diy_field diy_text text">
								<text>
									{{ o["gender_of_personnel"] }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="0 && $check_field('get','age_of_personnel')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>人员年龄</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/patrol_personnel/table','del') || $check_action('/patrol_personnel/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/patrol_personnel/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/patrol_personnel/table','set') || $check_action('/patrol_personnel/view','set') || $check_action('/patrol_personnel/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/patrol_personnel/get_list?like=0",
				url_del: "~/api/patrol_personnel/del?",
				...
				list: [],
                            // 人员性别列表
                list_gender_of_personnel: [{value:"",text:"全部"}],
                			}
		},
		methods: {
          search() {
